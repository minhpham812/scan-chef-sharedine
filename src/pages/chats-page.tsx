    ...
    <>
      <main className="flex-1 pb-8 lg:hidden">
        <header className="px-5 pb-2 pt-4">
          <h1 className="text-[18px] font-semibold text-foreground">チャット</h1>
        </header>

        <ChatThreadList threads={chatThreads} getThreadHref={thread => `/chat/${thread.id}`} />
      </main>
      ...
      <div className="hidden lg:flex lg:h-screen lg:flex-1 lg:overflow-hidden">
        <ChatThreadList
          threads={chatThreads}
          title="メッセージ"
          className="w-[320px] shrink-0 overflow-y-auto border-r border-[#e8e5dc] bg-white"
          selectedThreadId={selectedThreadId}
          onSelectThread={setSelectedThreadId}
        />

        <main className="flex min-w-0 flex-1 flex-col bg-background">
          {selectedThread ? <ChatConversationPane thread={selectedThread} /> : null}
        </main>
      </div>
    </>
  )
