...
const TABS = [
  { key: "all", label: "すべて", count: allBookings.length },
  {
    key: "request",
    label: "リクエスト",
    count: allBookings.filter((b) => b.status === "リクエスト").length,
  },
  {
    key: "upcoming",
    label: "今後の予定",
    count: allBookings.filter((b) => b.status === "承認済").length,
  },
  {
    key: "done",
    label: "完了",
    count: allBookings.filter((b) => ["完了", "キャンセル"].includes(b.status))
      .length,
  },
];
      ...
      <PageContent>
            ...
            filteredBookings.map((b, i) => (
              ...
              >
                ...
                <div className="mt-3 flex items-center justify-between border-t border-border pt-3">
                  <span className="text-[16px] font-bold text-[#8a7d52]">
                    {b.amount}
                  </span>
                </div>
              </div>
        ...
        <div className="mt-4 hidden lg:block">
          <div className="overflow-hidden rounded-xl border border-border bg-white">
            <Table>
              <TableHeader>
                <TableRow className="border-b border-border bg-[#fafaf7]">
                  <TableHead className="px-4 py-3 text-left text-[13px] font-semibold text-[#9e9a8f]">
                    お客様
                  </TableHead>
                  <TableHead className="px-4 py-3 text-left text-[13px] font-semibold text-[#9e9a8f]">
                    プラン
                  ...
                  <TableHead className="px-4 py-3 text-right text-[13px] font-semibold text-[#9e9a8f]">
                    アクション
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {filteredBookings.length === 0 ? (
                  <TableRow>
                    <TableCell
                      colSpan={7}
                      className="py-12 text-center text-[14px] text-[#9e9a8f]"
                    >
                      該当する予約はありません
                    </TableCell>
                  </TableRow>
                  ...
                  filteredBookings.map((b, i) => (
                    <TableRow
                      key={i}
                      className="border-b border-border last:border-b-0 transition-colors hover:bg-[#fafaf7]"
                    >
                      <TableCell className="px-4 py-3 text-[14px] font-medium text-[#2c2a24]">
                        {b.customer}
                      </TableCell>
                      <TableCell className="px-4 py-3 text-[14px] text-[#2c2a24]">
                        {b.plan}
                      </TableCell>
                      <TableCell className="px-4 py-3 text-[14px] text-[#9e9a8f]">
                        {b.date}
                      </TableCell>
                      <TableCell className="px-4 py-3 text-[14px] text-[#9e9a8f]">
                        {b.guests}
